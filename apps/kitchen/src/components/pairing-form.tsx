...
export function PairingForm() {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    try {
      await kitchenApi.me(token.trim());
    ...
    <form onSubmit={onSubmit} className="mx-auto w-full max-w-xl space-y-5">
      <label className="block space-y-2">
        ...
        <input
          ...
          placeholder="Paste the KITCHEN device token or scan admin QR"
          className="h-14 w-full rounded-lg border border-[var(--line)] bg-[var(--surface)] px-4 text-lg text-[var(--ink)] outline-none ring-[var(--heat)] placeholder:text-[var(--muted)] focus:ring-2"
          autoComplete="off"
          spellCheck={false}
          required
        />
      ...
      <Button type="submit" size="touch" className="w-full" disabled={pending}>
        {pending ? "Pairing…" : "Open kitchen display"}
      </Button>
    </form>
  );
}
